                ...
                <div className="w-36 h-full flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 rounded-l-lg border-r border-gray-200 relative overflow-hidden">
                    {product.icon ? (
                        <img
                            src={product.icon}
                            alt={product.name}
                            className="w-32 h-32 object-contain z-10 rounded-lg"
                        />
                    ) : (
                        <DefaultIcon className="w-24 h-24 z-10" />
                    )}
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex -space-x-1">
                                    {product.gallery_images.slice(0, 4).map((image, index) => (
                                        <div key={index} className="w-6 h-6 rounded border-2 border-white bg-gray-100 overflow-hidden shadow-sm">
                                            <img src={image} alt="" className="w-full h-full object-cover" />
                                        </div>
                                    ))}
                                    {product.gallery_images.length > 4 && (
                                        <div className="w-6 h-6 rounded border-2 border-white bg-gray-200 flex items-center justify-center shadow-sm">
